...
import br.com.totvs.cubo.api.model.*;
import br.com.totvs.cubo.api.model.dataquery.CuboDataQuery;
import br.com.totvs.cubo.api.model.dataquery.ZeusDataQuery;
import br.com.totvs.cubo.api.model.request.DemandaWSRequest;
import br.com.totvs.cubo.api.model.response.*;
...
public class CuboService extends GenericService {
    ...
    public Cubo adicionaAlteraCubo(Cubo cubo) throws ServiceException {
        try {
            ...
            Cubo updateEntity = new Cubo();
            updateEntity.setId(cubo.getId());

            if(find(updateEntity) != null){
                updateEntity = cubo;
                this.getDAO().getEm().merge(updateEntity);
                return updateEntity;
            }
            ...
            throw new ServiceException("Erro ao executar consulta na base de dados: " + e.getMessage());
        }
    }

    @Transactional
    public String targetDir() throws ServiceException {
        try {
    ...
    @Transactional
    public List<Cubo> getCubos() throws ServiceException {
        try {
            return (List<Cubo>)findList(CuboDataQuery.CUBOS);
        } catch (Exception e) {
            throw new ServiceException("Erro ao executar consulta na base de dados: " + e.getMessage());
        }
    }
    ...
    @Transactional
    public Cubo getCuboById(Integer id) throws ServiceException {
        try {
            Cubo c = new Cubo();
            c.setId(id);
            return (Cubo) find(c);
    ...
    public List<CuboWSResponse> retornaCubos() throws ServiceException {
        List<CuboWSResponse> response = new ArrayList<>();
        for(Cubo c: getCubos()){
            response.add(new CuboWSResponse(c));
        }
        return response;
    }
